...
//***************************************************************************************
  TFile *f = new TFile("MinvBB_Output.root","RECREATE");
  gStyle->SetOptFile(0);
  ...
  TH1D * _MinvBB_VBF_Hbb  = new TH1D("MinvBB_VBF_Hbb","MinvBB_VBF_Hbb",nbin,xmin,xmax);
  TH1D * _MinvBB_ggF_Hbb  = new TH1D("MinvBB_ggF_Hbb","MinvBB_ggF_Hbb",nbin,xmin,xmax);
  TH1D * _MinvBB_QCD  = new TH1D("MinvBB_QCD","MinvBB_QCD",nbin,xmin,xmax);
  TH1D * _MinvBB_Single_Top  = new TH1D("MinvBB_Single_Top","MinvBB_Single_Top",nbin,xmin,xmax);
  TH1D * _MinvBB_tt  = new TH1D("MinvBB_tt","MinvBB_tt",nbin,xmin,xmax);
  TH1D * _MinvBB_WJets  = new TH1D("MinvBB_WJets","MinvBB_WJets",nbin,xmin,xmax);
  TH1D * _MinvBB_ZJets  = new TH1D("MinvBB_ZJets","MinvBB_ZJets",nbin,xmin,xmax);
  TH1D * _MinvBB_DYJets = new TH1D("MinvBB_DYJets","MinvBB_DYJets",nbin,xmin,xmax);
  TH1D * _MinvBB_DATA   = new TH1D("MinvBB_DATA","MinvBB_DATA",nbin,xmin,xmax);
//***************************************************************************************
   ...
   for(int i=0;i<ntps[k]->GetEntries();i++)
    {
     ntps[k]->GetEntry(i);
     if(k==0)
      _MinvBB_VBF_Hbb->Fill(MyS_minvbb[k],MyS_weight[k]);
     if(k==1)
      _MinvBB_ggF_Hbb->Fill(MyS_minvbb[k],MyS_weight[k]);
    }
   ...
   for(int i=0;i<ntpb[k]->GetEntries();i++)
    {
     ntpb[k]->GetEntry(i);
     if(k<7)
      _MinvBB_QCD->Fill(MyB_minvbb[k],MyB_weight[k]);
     else if(k>=7 && k<11)
      _MinvBB_Single_Top->Fill(MyB_minvbb[k],MyB_weight[k]);
     else if(k>=11 && k<14)
      _MinvBB_tt->Fill(MyB_minvbb[k],MyB_weight[k]);
     else if(k>=14 && k<16)
      _MinvBB_WJets->Fill(MyB_minvbb[k],MyB_weight[k]);
     else if(k>=16 && k<19)
      _MinvBB_ZJets->Fill(MyB_minvbb[k],MyB_weight[k]);
     else if(k==19)
      _MinvBB_DYJets->Fill(MyB_minvbb[k],MyB_weight[k]);
     else if(k==20)
      _MinvBB_DATA->Fill(MyB_minvbb[k],MyB_weight[k]);
    }
