...
// First b-selection as highest btg-out
   for(int k=0;k<7;k++)
//    if(ptj[k]>PT_min_B1 && btgj_D[k]>btgD_MAX1 && btgj[k]>=btg_min_B1)      // DeepCSV b-tagging
    if(ptj[k]>PT_min_B1 && btgj[k]>btgD_MAX1 && btgj[k]>=btg_min_B1)      // CSV b-tagging
      {
       ptb1   = ptj[k];
       eb1    = ej[k];
       etab1  = etaj[k];
...
// Second b-selection as 2nd highest btg-out
   for(int k=0;k<7;k++)
//    if(ptj[k]>PT_min_B2 && ptj[k]!=ptb1 && btgj_D[k]>btgD_MAX2 && btgj[k]>=btg_min_B2)      // DeepCSV b-tagging
    if(ptj[k]>PT_min_B2 && ptj[k]!=ptb1 && btgj[k]>btgD_MAX2 && btgj[k]>=btg_min_B2)      // CSV b-tagging
      {
       ptb2   = ptj[k];
       eb2    = ej[k];
       etab2  = etaj[k];
...
// ******************************************************************************
  ...
  cout<<"Remaining after 1st b-jet selection (Always True as selection by MAX_BtagValue):                          "<<b1_Nofail<<endl;
  cout<<"Remaining after 2nd b-jet selection (Always True as selection by 2nd MAX_BtagValue):                      "<<b2_Nofail<<endl;
  ...
  cout<<"#2-bjets from H have PT>20, eta<4.7, pass PuLooseId, within 7-leading PT, dPHI<2.1"<<endl;
