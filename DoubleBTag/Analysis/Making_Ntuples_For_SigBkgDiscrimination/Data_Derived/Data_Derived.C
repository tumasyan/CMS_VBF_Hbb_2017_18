...
//********************************** Get NTuples and Create New **********************************
  ...
  TNtuple *ntuple_pt = new TNtuple(Datatype+"_pt",Datatype+"_pt","weight:PT1:PT2:PT3:PT4:PTb1:PTb2:PTq1:PTq2");
...
//     if(btgb2_D != btgb2_D)                  // There is one event with btgb2 = 0.893646 and btgb2_D =NaN
//       cout<<btgb2<<"   "<<btgb2_D<<endl;
   ...
   if(Datatype == "Sig")
     {
      ...
      if(Proc_Name[Nproc] == "VBFHToBB_M_125")
        weight = weight*rew2[(int)Tnpv];  // For VBF only
     }
    ...
    ntuple_pt -> Fill(weight,ptj[0],ptj[1],ptj[2],ptj[3],ptb1,ptb2,ptq1,ptq2);
