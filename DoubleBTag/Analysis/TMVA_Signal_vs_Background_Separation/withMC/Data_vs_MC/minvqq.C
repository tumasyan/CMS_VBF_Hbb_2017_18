...
//   TFile *f01 = TFile::Open("../../../Making_Ntuples_For_SigBkgDiscrimination/MC_Derived/NTuples_GluGluHToBB_M125_.root");
   TTree * ntps[1] = {(TTree*)f00->Get("Sig")};
...
//***************************************************************************************
  TFile *f = new TFile("MINVQQ_Output.root","RECREATE");
  gStyle->SetOptFile(0);
  ...
  TH1D * _MINVQQ_VBF_Hbb  = new TH1D("MINVQQ_VBF_Hbb","MINVQQ_VBF_Hbb",100,xmin,xmax);
  TH1D * _MINVQQ_ggF_Hbb  = new TH1D("MINVQQ_ggF_Hbb","MINVQQ_ggF_Hbb",100,xmin,xmax);
  TH1D * _MINVQQ_QCD  = new TH1D("MINVQQ_QCD","MINVQQ_QCD",100,xmin,xmax);
  TH1D * _MINVQQ_Single_Top  = new TH1D("MINVQQ_Single_Top","MINVQQ_Single_Top",100,xmin,xmax);
  TH1D * _MINVQQ_tt  = new TH1D("MINVQQ_tt","MINVQQ_tt",100,xmin,xmax);
  TH1D * _MINVQQ_WJets  = new TH1D("MINVQQ_WJets","MINVQQ_WJets",100,xmin,xmax);
  TH1D * _MINVQQ_ZJets  = new TH1D("MINVQQ_ZJets","MINVQQ_ZJets",100,xmin,xmax);
  TH1D * _MINVQQ_DYJets = new TH1D("MINVQQ_DYJets","MINVQQ_DYJets",100,xmin,xmax);
  TH1D * _MINVQQ_DATA   = new TH1D("MINVQQ_DATA","MINVQQ_DATA",100,xmin,xmax);
//***************************************************************************************
   ...
   for(int i=0;i<ntps[k]->GetEntries();i++)
    {
     ntps[k]->GetEntry(i);
     if(k==0)
      _MINVQQ_VBF_Hbb->Fill(MyS_minvqq[k],MyS_weight[k]);
//     if(k==1)
//      _MINVQQ_ggF_Hbb->Fill(MyS_minvqq[k],MyS_weight[k]);
    }
   ...
   for(int i=0;i<ntpb[k]->GetEntries();i++)
    {
     ntpb[k]->GetEntry(i);
     if(k<7)
      _MINVQQ_QCD->Fill(MyB_minvqq[k],MyB_weight[k]);
     else if(k>=7 && k<11)
      _MINVQQ_Single_Top->Fill(MyB_minvqq[k],MyB_weight[k]);
     else if(k>=11 && k<14)
      _MINVQQ_tt->Fill(MyB_minvqq[k],MyB_weight[k]);
     else if(k>=14 && k<16)
      _MINVQQ_WJets->Fill(MyB_minvqq[k],MyB_weight[k]);
     else if(k>=16 && k<19)
      _MINVQQ_ZJets->Fill(MyB_minvqq[k],MyB_weight[k]);
     else if(k==19)
      _MINVQQ_DYJets->Fill(MyB_minvqq[k],MyB_weight[k]);
     else if(k==20)
      _MINVQQ_DATA->Fill(MyB_minvqq[k],MyB_weight[k]);
    }
